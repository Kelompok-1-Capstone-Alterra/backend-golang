...
func GetOverview(c echo.Context) error {
	...
	if err := config.DB.
		Raw("SELECT * FROM info_weathers iw WHERE (iw.created_at, iw.updated_at) IN (SELECT MAX(created_at), MAX(updated_at) FROM info_weathers WHERE location = iw.location) ORDER BY GREATEST(iw.created_at, iw.updated_at) DESC LIMIT 10").
